"""Test NVIDIA API models directly for monster generation"""
...
NVIDIA_MODELS = [
    {"model_id": "z-ai/glm4.7", "name": "GLM 4.7"},
    {"model_id": "minimaxai/minimax-m2.1", "name": "MiniMax M2.1"},
    ...
    {"model_id": "qwen/qwen3-next-80b-a3b-thinking", "name": "Qwen 3 Next 80B"},
    {"model_id": "bytedance/seed-oss-36b-instruct", "name": "Seed OSS 36B"},
]

# Rate limiting for NVIDIA (conservative to avoid rate limits)
NVIDIA_RATE_LIMIT_DELAY = (
    4.0  # seconds between requests (increased to avoid rate limits with more models)
)
...
def run_nvidia_tests(
    ...
    print("=" * 60)
    print("NVIDIA API Model Testing")
    print("=" * 60)
...
def main():
    ...
    output_file = os.path.join(
        os.path.dirname(__file__), f"nvidia_test_results_{timestamp}.json"
    )
