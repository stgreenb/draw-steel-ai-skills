...
- **Characteristics:** Based on echelon (Level 1-2=+1, 3-4=+2, 5-6=+3, 7-8=+4, 9-10=+5)
...
### Elite Brute (Level 3-4, Echelon 1)
| Might | Agility | Reason | Intuition | Presence |
|-------|---------|--------|-----------|----------|
| +2 to +3 | 0 to +1 | -2 to -1 | 0 to +1 | 0 to +0 |

### Elite Harrier (Level 3-4, Echelon 1)
| Might | Agility | Reason | Intuition | Presence |
|-------|---------|--------|-----------|----------|
| +1 to +2 | +2 to +3 | -1 to +1 | +1 to +2 | 0 to +0 |

### Elite Controller (Level 3-4, Echelon 1)
| Might | Agility | Reason | Intuition | Presence |
|-------|---------|--------|-----------|----------|
| 0 to +1 | 0 to +1 | +1 to +2 | +1 to +2 | 0 to +1 |

### Solo (Level 5-6, Echelon 2) - Add +1 bonus
| Might | Agility | Reason | Intuition | Presence |
|-------|---------|--------|-----------|----------|
| +3 to +4 | +2 to +3 | +1 to +2 | +2 to +3 | +2 to +3 |

### Leader (Level 5-6, Echelon 2) - Add +1 bonus
| Might | Agility | Reason | Intuition | Presence |
|-------|---------|--------|-----------|----------|
...
**Note:** Force movement is COMMON in Draw Steel monsters. Often a monster will have an ability with push, pull, or slide effects to make combat more dynamic and tactical.

**Note:** The tables below are GUIDELINES, not strict rules. Actual monsters show significant variation based on ability design, organization, and specific creature concepts.
...
|-------------|---------|-----------------|-----------------|------------------|
| Level 1-2 | 0 | 1/1/1 | 1/2/2 | 1/2/2 |
| Level 3-4 | 1 | 1/2/2 | 2/3/4 | 2/3/4 |
| Level 5-6 | 2 | 2/2/3 | 3/4/5 | 3/4/5 |
| Level 7-8 | 3 | 2/3/3 | 4/5/6 | 4/5/6 |
| Level 9-10 | 4 | 3/4/4 | 5/6/7 | 5/6/7 |
...
### Force Movement Quick Tips

**When to Use Force Movement:**
- **Push**: Brutes pushing enemies away, Controllers pushing groups
- **Pull**: Harriers pulling targets into melee, Artillery pulling into range
- **Slide**: Controllers repositioning enemies, any ranged attack with positional effects
- **Vertical**: Throws, drops, levitation effects

**Tier-Based Patterns:**
- **Tier 1**: Small movement (1-2 squares) - quick repositioning
- **Tier 2**: Medium movement (2-4 squares) - tactical repositioning
- **Tier 3**: Large movement (4-6 squares) - dramatic repositioning

**Malice Features with Force Movement:**
- Common pattern: "Spend X Malice to [pull/push/slide] target Y squares"
- Solo/Leader malice features often include force movement for crowd control

**Movement with Attack Pattern:**
"The [creature] [jumps/shifts/flies] up to [X] squares before or after making this strike."

**Examples from Official Monsters:**
- Level 1 Goblin Spinecleaver (Brute): push 1/3/4 (tier-scaled push)
- Level 1 Angulotl Daybringer (Leader): pull 4 (fixed value, malice feature)
- Level 3 Bredbeddle (Solo): push 1 (small push on movement ability)
...
| Ambusher | Pull | 2/3/4 |
| Defender | Push | 1-2 (protecting allies) |
| Support | Slide | 1-3 (repositioning allies) |
...
**CRITICAL: Exactly ONE Signature Ability**
- The **first ability listed** is the ONLY signature ability
- Mark it as `"category": "signature"` in Foundry JSON
- Label it as "Signature Ability" in markdown stat block
- **NO other abilities** should be marked as signature
...
## Step 8b: Minion With Captain Trait (MANDATORY for Minions)

**CRITICAL:** All Minion organizations MUST include a "With Captain" trait.

**Rule (Monster Basics.md line 463):**
> "While a minion squad has a captain, each minion in the squad gains the benefits noted at the 'With Captain' entry on their stat block."

**Role-Appropriate Captain Bonuses:**

| Role | With Captain Bonus |
|------|-------------------|
| **Brute** | +1 damage bonus to strikes |
| **Harrier** | +1-2 bonus to speed |
| **Defender** | +1 bonus to Stability |
| **Ambusher** | +1 bonus to attack rolls (first attack) |
| **Artillery** | +1 to ranged damage rolls |
| **Controller** | +1 to potency of abilities |
| **Hexer** | +1 to potency of abilities |
| **Support** | +1 to aid another checks |
...
### With Captain
When this minion is in a squad with a captain, they gain a [+1 damage bonus to strikes / +2 bonus to speed / etc.].
```
...
**Official Example:** Goblin Spinecleaver has "+1 damage bonus to strikes" as its With Captain benefit.
...
- [ ] **Exactly ONE Signature Ability:** Only the first ability is marked as "Signature Ability"
- [ ] **Minion With Captain:** If organization = Minion, include a "With Captain" trait with role-appropriate bonus
- [ ] **Malice Features Present:** All non-trivial monsters have 2-3 malice features (Platoon/Elite/Minion/Horde = 2, Leader/Solo = 2-3)
- [ ] **NO D&D Terminology:** No "vs. AC", "Armor Class", "Fortitude save", "Reflex save", "Will save", "DC", "HP", "d20"
- [ ] **Positive Potencies Only:** All potencies use POSITIVE values (0-6), never negative (e.g., "A < 1" ✓, "A < -1" ✗)
- [ ] **Standard Table Format:** Uses the official stat block table with Size/Speed/Stamina/Stability/Free Strike columns
...
**CRITICAL: Malice Features Are MANDATORY**
- ALL monsters MUST have malice features unless they are extremely simple (trivial minions with 1-2 abilities)
- **Platoon/Elite:** Generate 2 malice features minimum
- **Minion/Horde:** Generate 2 malice features minimum (shared malice pool)
- **Leader:** Generate 2-3 malice features + 3 villain actions
- **Solo:** Generate 3 malice features + 3 villain actions
- **NEVER omit malice features** - they are core to Draw Steel combat mechanics
...
1. **NO D&D terminology:** No "vs. AC", "Armor Class", "HP", "hit points", "d20", "DC", "Fortitude save", "Reflex save", "Will save"
2. **Use 2d10:** Power rolls use `2d10 + characteristic` (NOT "vs AC")
3. **Potencies are POSITIVE:** Write as `[Char] < Value` where Value is 0-6 (e.g., `A < 1`, `M < 3`). NEVER use negative potencies like "A < -1"
4. **Tier ranges:** ≤11 (T1), 12-16 (T2), 17+ (T3)
5. **Free Strike = T1 damage:** This is the definitive rule
6. **Horde/Minion damage:** Always divide by 2 after calculation
7. **All stats use ceil():** Round UP for calculations (EV, Stamina, Damage)
8. **Division uses round DOWN:** Always round down when dividing (for Horde/Minion)
9. **Natural 19-20:** Always tier 3, can be a critical hit
10. **Characteristics:** Range from -5 to +5, where 0 = average human
11. **Edges and Banes:** Edge = +2, Bane = -2, they cancel out
12. **Potencies:** Write as `[Char] < Value` (e.g., `A < 1, prone`). Target resists if their score >= value. Potency values are always POSITIVE (0-6).
13. **Strike Bonus:** Add highest characteristic to strike damage
14. **Echelon scaling:** Highest characteristic = 1 + echelon (Level 1-2=+1, 3-4=+2, 5-6=+3, 7-8=+4, 9-10=+5)
15. **Leader/Solo bonuses:** +1 to highest characteristic (max +5), +1 to all potencies
16. **Target counts:** Normal=1, Elite/Leader/Solo=2
17. **Match the MCDM format exactly:** Use the table format, icons, and structure shown
18. **Use common damage types:** fire, corruption, poison, psychic, holy
19. **Use common conditions:** prone, slowed, bleeding, dazed, restrained, weakened
20. **Most minions are 1M size, not 1S:** Follow Bestiary patterns
21. **Minion With Captain trait:** ALL minions must have a "With Captain" trait with role-appropriate bonus (+1 damage for Brutes, +1-2 speed for Harriers, etc.)
22. **Malice features are MANDATORY:** All monsters except trivial minions must have 2-3 malice features
23. **Standard table format REQUIRED:** Always use the stat block table with columns for Size/Speed/Stamina/Stability/Free Strike, Immunities/Movement/With Captain/Weaknesses, and characteristics

## Common Pitfalls to Avoid (CRITICAL)

### ❌ NEVER Use These Terms:
- "vs. AC" → Use "Power Roll + [Characteristic]" instead
- "Armor Class" or "AC" → Use potencies like "A < 1"
- "Fortitude save", "Reflex save", "Will save" → Use characteristic-based potencies
- "DC" → Use potency thresholds like "M < 2"
- "HP" or "hit points" → Use Stamina
- "d20" → Use 2d10 for power rolls
- "sickened" → Not a valid Draw Steel condition
- Negative potencies like "A < -1" → Potencies are always POSITIVE (0-6)

### ✅ ALWAYS Include These:
- **Malice Features:** All non-trivial monsters MUST have 2-3 malice features
- **With Captain Trait:** ALL minions MUST have this trait with role-appropriate bonus
- **Standard Table Format:** Use the official stat block table (see template above)
- **Positive Potencies:** All potency values are 0-6, never negative
- **Valid Conditions:** Only use approved conditions (prone, slowed, bleeding, dazed, restrained, weakened, frightened, grabbed, hexed)

### ❌ Common Mistakes:
1. **Omitting malice features** → EVERY monster except trivial minions needs 2-3 malice features
2. **Using negative potencies** → "A < -1" is invalid, use "A < 1" instead
3. **Using D&D saving throws** → "Fortitude save" is wrong, use "M < 2" instead
4. **Missing With Captain for minions** → All minions need this trait
5. **Wrong table format** → Must use the official 5-row table format
6. **Invalid conditions** → "sickened" doesn't exist in Draw Steel
